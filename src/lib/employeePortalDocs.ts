...
// operations go through the `employee-portal` Edge Function (service role),
// authenticated by the durable portal token (see portalSession.ts).
...
async function call<T>(body: Record<string, unknown>): Promise<{ data: T | null; error: string | null }> {
  ...
  const { data, error } = await fdb.functions.invoke('employee-portal', { body: { token, ...body } });
  if (error) {
...
export async function listPortalDocuments(): Promise<{ employment: PortalDocument[]; personal: PortalDocument[]; error: string | null }> {
  const { data, error } = await call<{ employment: PortalDocument[]; personal: PortalDocument[] }>({ action: 'list_documents' });
  return { employment: data?.employment ?? [], personal: data?.personal ?? [], error };
}

export async function uploadPersonalDoc(file: File, docType: string, signature: SignatureData): Promise<{ error: string | null }> {
...
export async function openPortalDocument(id: string): Promise<string | null> {
  const { data } = await call<{ url: string }>({ action: 'signed_url', id });
